...
public class DetectorThread extends Thread {
		...
		try {
			...
			while (_thread == thisThread) {
				...
				if (buffer != null) {
					...
					if (numWhistles >= whistlePassScore) { // clear buffer
						initBuffer();
						onWhistleOrClapDetected();
					}
					...
					// no sound detected
					// Log.e("Whistle and clap detector", "no sound detected");
					if (whistleResultList.getFirst()) {
						numWhistles--;
					}
					whistleResultList.removeFirst();
	...
	private void onWhistleOrClapDetected() {
	...
	public void removeOnSignalsDetectedListener() {
